...
import { Upload, Download, Trash2, X, ImageIcon, Search } from "lucide-react"
...
export function HudsManager() {
  ...
  const [imageModal, setImageModal] = useState<{ show: boolean; hud: HudFile | null }>({
    show: false,
    hud: null,
  })
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === "Escape") {
        setImageModal({ show: false, hud: null })
  ...
  // Fetch HUD files with React Query
  const { data: filesData, isLoading } = useQuery({
    queryKey: ['hud-files'],
    queryFn: async () => {
      const res = await fetch("/api/list-huds")
  ...
  const uploadMutation = useMutation({
    mutationFn: async (formData: FormData) => {
      const res = await fetch("/api/upload-hud", {
        method: "POST",
        body: formData,
      })
      if (!res.ok) {
        ...
        if (contentType && contentType.includes("application/json")) {
          ...
          throw new Error(data.error || "Error al subir el HUD")
        }
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['hud-files'] })
      setUploadModal(false)
  ...
  const deleteMutation = useMutation({
    mutationFn: async (fileName: string) => {
      const res = await fetch(`/api/delete-hud?name=${encodeURIComponent(fileName)}`, {
        method: "DELETE",
      })
      if (!res.ok) {
        const data = await res.json()
  ...
  const steamIds = useMemo(() => {
    return files.map((f: HudFile) => f.userId.replace('steam_', '')).filter(Boolean)
  }, [files])
  ...
  return (
    <div className="space-y-4">
        ...
        >
          ...
          >
            ...
            <div className="relative min-h-0 bg-[#0c0c0e] flex items-center justify-center overflow-hidden">
              <img
                src={imageModal.hud.previewImage || "/branding/logo.png"}
                alt={imageModal.hud.name}
            ...
            <div className="flex-shrink-0 bg-card border-t border-foreground/[0.06] px-4 py-3 flex items-center justify-between gap-4">
              <div className="flex items-center gap-3 min-w-0">
                <PlayerAvatar steamId={imageModal.hud.userId.replace('steam_', '')} playerName={imageModal.hud.username} size="sm" />
                <div className="min-w-0">
                  ...
                  <div className="flex items-center gap-2.5 text-[10px] text-foreground/40 mt-0.5">
                    <span>{imageModal.hud.username}</span>
                    <span className="flex items-center gap-1 tabular-nums">
                      <Download className="h-2.5 w-2.5" />
                      {imageModal.hud.downloads}
                    </span>
                    <span>{formatRelativeDate(imageModal.hud.uploadDate)}</span>
                  </div>
                  {imageModal.hud.description && (
                    <p className="text-[10px] text-foreground/30 mt-1 truncate max-w-md">{imageModal.hud.description}</p>
                  )}
                </div>
              </div>
              <button
                onClick={() => {
                  ...
                  link.href = `/api/get-hud?name=${encodeURIComponent(imageModal.hud!.name)}`
                  link.download = imageModal.hud!.name
      ...
      <div className="mt-3 text-[10px] font-bold uppercase tracking-wider text-foreground/25">
        {isLoading ? "..." : `${filteredAndSortedFiles.length} HUDs`}
      </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5 mt-3">
          {filteredAndSortedFiles.map((file, index) => (
            <div
              ...
              onClick={() => setImageModal({ show: true, hud: file })}
            >
              {/* Preview Image */}
                ...
                <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-center justify-center">
                  <div className="flex gap-2" onClick={(e) => e.stopPropagation()}>
                    <button
                      onClick={() => {
                        const link = document.createElement("a")
                        link.href = `/api/get-hud?name=${encodeURIComponent(file.name)}`
